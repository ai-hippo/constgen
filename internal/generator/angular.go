...
// AngularGenerator emits a TypeScript file for Angular usage.
type AngularGenerator struct{}

func (g AngularGenerator) FileName(spec model.Spec, profile string) string {
    return fmt.Sprintf(
        "%s.ts",
        spec.Name,
    )
...
func (g AngularGenerator) Generate(spec model.Spec, profile string, lang string) string {
    ...
    name := strings.TrimSuffix(g.FileName(spec, profile), ".ts")
        ...
        if fm, ok2 := raw.(map[string]interface{}); ok2 {
            for fname, val := range fm {
                ...
                if sig != "" {
                    ...
                    b.WriteString(fmt.Sprintf("  '%s': (value: any) => {\n", fname))
