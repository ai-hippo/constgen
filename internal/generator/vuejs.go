...
// VueGenerator emits a JavaScript module for Vue (ES module).
type VueGenerator struct{}

func (g VueGenerator) FileName(spec model.Spec, profile string) string {
    return fmt.Sprintf(
        "%s.js",
        spec.Name,
    )
...
func (g VueGenerator) Generate(spec model.Spec, profile string, lang string) string {
    ...
    // export a named constant matching the filename base
